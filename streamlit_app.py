...
if submitted:
    ...
    elif risk_category == "PREMIUM_LOADING_REQUIRED":

        st.warning(
            "🟠 PREMIUM LOADING REQUIRED"
        )

    elif risk_category == "MANDATORY_MEDICAL_TESTS":

        st.error(
            "🔴 MEDICAL TESTS REQUIRED"
        )

    elif risk_category == "KNOWLEDGE_QUERY":

        st.info(
            "🔵 KNOWLEDGE QUERY"
        )

    elif risk_category == "DECLINED":
    ...
    if is_knowledge_query:
        ...
        with tab1:
            ...
            if ulip:
                ...
                col1.metric(
                    "Investment",
                    f"₹{ulip['investment_amount']:,.0f}"
                )

                col2.metric(
                    "4% Illustration",
                    f"₹{ulip['illustration_4_percent']:,.0f}"
                )

                col3.metric(
                    "8% Illustration",
                    f"₹{ulip['illustration_8_percent']:,.0f}"
                )
        ...
        with tab3:
            ...
            if docs:
                ...
                st.info(
                    "No underwriting documents retrieved."
                )
